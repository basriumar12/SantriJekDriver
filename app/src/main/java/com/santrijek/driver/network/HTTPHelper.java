package com.santrijek.driver.network;
...
import com.santrijek.driver.model.Feedback;
...
//                Log.d("Auth", base64EncodedCredentials);
    ...
    public void verifikasiTopUp(final JSONObject dataOrder, final NetworkActionResult actionResult){
        JsonObjectRequest jsObjRequest = new JsonObjectRequest
...
//                Log.d("Auth", base64EncodedCredentials);
    ...
    public void getTransaksiMstore(JSONObject dataOrder, final NetworkActionResult actionResult){
        JsonObjectRequest jsObjRequest = new JsonObjectRequest
                (Request.Method.POST, MyConfig.URL_DRIVER_GET_TRANSAKSI_MSTORE, dataOrder, new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        Log.d(MyConfig.TAG_response+" JSON", response.toString());
                        actionResult.onSuccess(response);
    ...
    public void getTransaksiMlaundry(JSONObject dataOrder, final NetworkActionResult actionResult){
        JsonObjectRequest jsObjRequest = new JsonObjectRequest
                (Request.Method.POST, MyConfig.URL_DRIVER_GET_TRANSAKSI_MLAUNDRY, dataOrder, new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        Log.d(MyConfig.TAG_response+" JSON", response.toString());
                        Log.e("kesalahan Httpjelper",response+"");
                        actionResult.onSuccess(response);
                    }
                }, new Response.ErrorListener() {
                    @Override
    ...
    public void driverFinishMlaundry(JSONObject dataOrder, final NetworkActionResult actionResult){
        JsonObjectRequest jsObjRequest = new JsonObjectRequest
                (Request.Method.POST, MyConfig.URL_DRIVER_FINISH_MLAUNDRY, dataOrder, new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        Log.d(MyConfig.TAG_response+" JSON", response.toString());
                        actionResult.onSuccess(response);
        ...
        if (json.equals("")) {
            ...
            try {
                ...
                if (jObject.getString("message").equals("found")) {
                    ...
                    KendaraanPreference kp = new KendaraanPreference(context);
...
//            Log.d("JSON_feedback", jObject.toString());
    ...
    public static ArrayList<RiwayatTransaksi> parseRiwayatTransaksi(JSONObject jObject) {
        ArrayList<RiwayatTransaksi> arrRiwayat = new ArrayList<>();
        try {
//            Log.d("JSON_riwayat_transaksi", jObject.toString());
            if (jObject.getString("message").equals("success")) {
                ...
                for(int i=0; i<jArray.length(); i++){
                    ...
                    riwayat.alamat_asal = oneObject.getString("alamat_asal");
                    riwayat.alamat_tujuan = oneObject.getString("alamat_tujuan");
                    ...
                    riwayat.biay_akhir = oneObject.getString("biaya_akhir");
...
//                sp.updateKerja(swit);
    ...
    public static Transaksi parseDataMfood(Transaksi transaksi, JSONObject jObject) {
        ...
        try {
            if (jObject.getString("message").equals("success")){
                ...
                transaksi.pakai_mpay = oneObject.getString("pakai_mpay");
                transaksi.harga_akhir = oneObject.getString("harga_akhir");
                transaksi.nama_resto = oneObject.getString("nama");
    ...
    public static Transaksi parseDataMstore(Transaksi transaksi, JSONObject jObject) {
        Log.d("JSON_data_mstore", jObject.toString());
        try {
            if (jObject.getString("message").equals("success")){
                JSONArray jArray = jObject.getJSONArray("data_transaksi");
                JSONObject oneObject = jArray.getJSONObject(0);
...
//                transaksi.estimasi_biaya = oneObject.getInt("estimasi_biaya");
                transaksi.harga_akhir = oneObject.getString("harga_akhir");
                transaksi.nama_toko = oneObject.getString("nama");
                transaksi.total_biaya = oneObject.getInt("total_biaya");
                transaksi.telepon_toko = oneObject.getString("kontak_telepon");
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public static Transaksi parseDataMlaundry(Transaksi transaksi, JSONObject jObject) {
        Log.d("JSON_data_mlaundry", jObject.toString());
        try {
            if (jObject.getString("message").equals("success")){
                JSONArray jArray = jObject.getJSONArray("data_transaksi");
                JSONObject oneObject = jArray.getJSONObject(0);
                ...
                transaksi.pakai_mpay = oneObject.getString("pakai_mpay");
                transaksi.harga_akhir = oneObject.getString("harga_akhir");
                transaksi.nama_laundry = oneObject.getString("nama");
                transaksi.total_biaya = oneObject.getInt("total_biaya");
                transaksi.telepon_laundry = oneObject.getString("kontak_telepon");
            }
        } catch (JSONException e) {
            e.printStackTrace();
            Log.e("kesalahan parse",e+"");
        }
        return transaksi;
    }
...
//                transaksi.harga_akhir = oneObject.getString("harga_akhir");
    ...
    public static ArrayList<BarangBelanja> parseBarangBelanja(JSONObject jObject) {
        Log.d("JSON_riwayat_transaksi", jObject.toString());
        ArrayList<BarangBelanja> arrBarang = new ArrayList<>();
        try {
            if (jObject.getString("message").equals("success")) {
                JSONArray jArray = jObject.getJSONArray("list_barang");
                for(int i=0; i<jArray.length(); i++){
                    BarangBelanja barang_belanja = new BarangBelanja();
                    JSONObject oneObject = jArray.getJSONObject(i);
    ...
    public static ArrayList<MakananBelanja> parseMakananBelanja(JSONObject jObject) {
        Log.d("JSON_makanan_belanja", jObject.toString());
        ArrayList<MakananBelanja> arrBarang = new ArrayList<>();
        try {
            if (jObject.getString("message").equals("success")) {
                JSONArray jArray = jObject.getJSONArray("list_barang");
                for(int i=0; i<jArray.length(); i++){
                    MakananBelanja barang_belanja = new MakananBelanja();
                    JSONObject oneObject = jArray.getJSONObject(i);
    ...
    public static ArrayList<StoreBelanja> parseStoreBelanja(JSONObject jObject) {
        Log.d("JSON_store_belanja", jObject.toString());
        ArrayList<StoreBelanja> arrBarang = new ArrayList<>();
        try {
            if (jObject.getString("message").equals("success")) {
                JSONArray jArray = jObject.getJSONArray("list_barang");
                for(int i=0; i<jArray.length(); i++){
                    StoreBelanja barang_belanja = new StoreBelanja();
                    JSONObject oneObject = jArray.getJSONObject(i);
                    barang_belanja.id_barang = oneObject.getInt("id_barang");
                    barang_belanja.nama_barang = oneObject.getString("nama_barang");
                    barang_belanja.jumlah_barang = oneObject.getInt("jumlah");
                    barang_belanja.harga_barang = oneObject.getInt("harga");
                    barang_belanja.catatan_barang = oneObject.getString("catatan");
                    arrBarang.add(barang_belanja);
                }
            }
        } catch (JSONException e) {
    ...
    public static ArrayList<LaundryBelanja> parseLaundryBelanja(JSONObject jObject) {
        Log.d("JSON_laundry_belanja", jObject.toString());
        ArrayList<LaundryBelanja> arrBarang = new ArrayList<>();
        try {
            if (jObject.getString("message").equals("success")) {
                JSONArray jArray = jObject.getJSONArray("list_barang");
                for(int i=0; i<jArray.length(); i++){
                    LaundryBelanja laundry_belanja = new LaundryBelanja();
                    JSONObject oneObject = jArray.getJSONObject(i);
                    laundry_belanja.id_item_laundry = oneObject.getInt("id_item_laundry");
                    laundry_belanja.nama_menu = oneObject.getString("nama_menu");
                    laundry_belanja.jumlah_laundry = oneObject.getInt("jumlah");
                    laundry_belanja.harga_laundry = oneObject.getInt("harga");
                    laundry_belanja.catatan_laundry = oneObject.getString("catatan");
                    arrBarang.add(laundry_belanja);
                }
            }
        } catch (JSONException e) {
            e.printStackTrace();
    ...
    public static ArrayList<DestinasiMbox> parseDestinasiMbox(JSONObject jObject) {
        Log.d("JSON_destinasi_mbox", jObject.toString());
        ArrayList<DestinasiMbox> arrDestinasi = new ArrayList<>();
        try {
            if (jObject.getString("message").equals("success")){
                JSONArray jArray = jObject.getJSONArray("list_destinasi");
                for(int i=0; i<jArray.length(); i++){
                    DestinasiMbox destinasiMbox = new DestinasiMbox();
                    JSONObject oneObject = jArray.getJSONObject(i);
                    destinasiMbox.id = oneObject.getInt("id");
                    destinasiMbox.id_transaksi = oneObject.getInt("id_transaksi");
                    destinasiMbox.lokasi = oneObject.getString("lokasi");
                    destinasiMbox.detail_lokasi = oneObject.getString("detail_lokasi");
                    destinasiMbox.latitude = oneObject.getString("latitude");
                    destinasiMbox.longitude = oneObject.getString("longitude");
                    destinasiMbox.nama_penerima = oneObject.getString("nama_penerima");
                    destinasiMbox.telepon_penerima = oneObject.getString("telepon_penerima");
                    destinasiMbox.instruksi = oneObject.getString("instruksi");
                    arrDestinasi.add(destinasiMbox);
                }
            }
        } catch (JSONException e) {
            e.printStackTrace();
