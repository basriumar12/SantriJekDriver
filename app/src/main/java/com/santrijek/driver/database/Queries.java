package com.santrijek.driver.database;
...
import com.santrijek.driver.model.Chat;
...
public class Queries {
    ...
    public void insertInProgressTransaksi(Transaksi transaksi) {
        ...
        values.put(DBHandler.COLUMN_NAMA_LAUNDRY, transaksi.nama_laundry);
    ...
    public void insertRiwayatTransaksi(ArrayList<RiwayatTransaksi> riwayatTransaksis) {
        for (RiwayatTransaksi instance : riwayatTransaksis) {
            ContentValues values = new ContentValues();
            ...
            values.put(DBHandler.COLUMN_ALAMAT_ASAL, instance.alamat_asal);
            values.put(DBHandler.COLUMN_ALAMAT_TUJUAN, instance.alamat_tujuan);
            values.put(DBHandler.COLUMN_BIAYA_AKHIR, instance.biay_akhir);
    ...
    public ArrayList<RiwayatTransaksi> getAllRiwayatTransaksi() {
        ArrayList<RiwayatTransaksi> list = new ArrayList<>();
        ...
        if (mCursor.getCount() > 0){
            if (!mCursor.isAfterLast()) {
                do {
                    RiwayatTransaksi entry = new RiwayatTransaksi();
                    ...
                    entry.biay_akhir = mCursor.getString(mCursor.getColumnIndex(DBHandler.COLUMN_BIAYA_AKHIR));
    ...
    public Transaksi getInProgressTransaksi() {
        ...
        if (mCursor.getCount() > 0){
            ...
            entry.nama_laundry = mCursor.getString(mCursor.getColumnIndex(DBHandler.COLUMN_NAMA_LAUNDRY));
    ...
    public void insertBarangBelanja(ArrayList<BarangBelanja> barang_belanjas) {
        for (BarangBelanja instance : barang_belanjas) {
            ContentValues values = new ContentValues();
    ...
    public void insertMakananBelanja(ArrayList<MakananBelanja> makananBelanjas) {
        for (MakananBelanja instance : makananBelanjas) {
            ContentValues values = new ContentValues();
    ...
    public void insertStoreBelanja(ArrayList<StoreBelanja> makananBelanjas) {
        for (StoreBelanja instance : makananBelanjas) {
            ContentValues values = new ContentValues();
            values.put(DBHandler.COLUMN_ID_BARANG, instance.id_barang);
            values.put(DBHandler.COLUMN_NAMA_BARANG, instance.nama_barang);
            values.put(DBHandler.COLUMN_JUMLAH_BARANG, instance.jumlah_barang);
            values.put(DBHandler.COLUMN_HARGA_BARANG, instance.harga_barang);
            values.put(DBHandler.COLUMN_CATATAN_BARANG, instance.catatan_barang);
            writableDatabase.insert(DBHandler.TABLE_STORE_BELANJA, null, values);
        }
    }

    public void insertLaundryBelanja(ArrayList<LaundryBelanja> makananBelanjas) {
        for (LaundryBelanja instance : makananBelanjas) {
            ContentValues values = new ContentValues();
            values.put(DBHandler.COLUMN_ID_ITEM_LAUNDRY, instance.id_item_laundry);
            values.put(DBHandler.COLUMN_NAMA_MENU, instance.nama_menu);
            values.put(DBHandler.COLUMN_JUMLAH_LAUNDRY, instance.jumlah_laundry);
            values.put(DBHandler.COLUMN_HARGA_LAUNDRY, instance.harga_laundry);
            values.put(DBHandler.COLUMN_CATATAN_LAUNDRY, instance.catatan_laundry);
            writableDatabase.insert(DBHandler.TABLE_LAUNDRY_BELANJA, null, values);
        }
    }

    public void insertDestinasiMbox(ArrayList<DestinasiMbox> destinasiMboxes) {
        for (DestinasiMbox instance : destinasiMboxes) {
            ContentValues values = new ContentValues();
            values.put(DBHandler.COLUMN_ID, instance.id);
            values.put(DBHandler.COLUMN_ID_TRANSAKSI, instance.id_transaksi);
    ...
    public ArrayList<BarangBelanja> getAllBarangBelanja() {
        ArrayList<BarangBelanja> list = new ArrayList<>();
        Cursor mCursor = readableDatabase.rawQuery("SELECT * FROM " + DBHandler.TABLE_BARANG_BELANJA, null);
        mCursor.moveToFirst();
        if (mCursor.getCount() > 0){
            if (!mCursor.isAfterLast()) {
                do {
                    BarangBelanja entry = new BarangBelanja();
    ...
    public ArrayList<StoreBelanja> getAllStoreBelanja() {
        ArrayList<StoreBelanja> list = new ArrayList<>();
        Cursor mCursor = readableDatabase.rawQuery("SELECT * FROM " + DBHandler.TABLE_STORE_BELANJA, null);
        mCursor.moveToFirst();
        if (mCursor.getCount() > 0){
            if (!mCursor.isAfterLast()) {
                do {
                    StoreBelanja entry = new StoreBelanja();
                    entry.id_barang = mCursor.getInt(mCursor.getColumnIndex(DBHandler.COLUMN_ID_BARANG));
                    entry.nama_barang = mCursor.getString(mCursor.getColumnIndex(DBHandler.COLUMN_NAMA_BARANG));
                    entry.jumlah_barang = mCursor.getInt(mCursor.getColumnIndex(DBHandler.COLUMN_JUMLAH_BARANG));
                    entry.harga_barang = mCursor.getInt(mCursor.getColumnIndex(DBHandler.COLUMN_HARGA_BARANG));
                    entry.catatan_barang = mCursor.getString(mCursor.getColumnIndex(DBHandler.COLUMN_CATATAN_BARANG));
...
//                    entry.isChecked = mCursor.getInt(mCursor.getColumnIndex(DBHandler.COLUMN_IS_CHECKED));
    ...
    public ArrayList<LaundryBelanja> getAllLaundryBelanja() {
        ArrayList<LaundryBelanja> list = new ArrayList<>();
        Cursor mCursor = readableDatabase.rawQuery("SELECT * FROM " + DBHandler.TABLE_LAUNDRY_BELANJA, null);
        mCursor.moveToFirst();
        if (mCursor.getCount() > 0){
            if (!mCursor.isAfterLast()) {
                do {
                    LaundryBelanja entry = new LaundryBelanja();
                    entry.id_item_laundry = mCursor.getInt(mCursor.getColumnIndex(DBHandler.COLUMN_ID_ITEM_LAUNDRY));
                    entry.nama_menu = mCursor.getString(mCursor.getColumnIndex(DBHandler.COLUMN_NAMA_MENU));
                    entry.jumlah_laundry = mCursor.getInt(mCursor.getColumnIndex(DBHandler.COLUMN_JUMLAH_LAUNDRY));
                    entry.harga_laundry = mCursor.getInt(mCursor.getColumnIndex(DBHandler.COLUMN_HARGA_LAUNDRY));
                    entry.catatan_laundry = mCursor.getString(mCursor.getColumnIndex(DBHandler.COLUMN_CATATAN_LAUNDRY));
...
//                    entry.isChecked = mCursor.getInt(mCursor.getColumnIndex(DBHandler.COLUMN_IS_CHECKED));
    ...
    public ArrayList<MakananBelanja> getAllMakananBelanja() {
        ArrayList<MakananBelanja> list = new ArrayList<>();
        Cursor mCursor = readableDatabase.rawQuery("SELECT * FROM " + DBHandler.TABLE_MAKANAN_BELANJA, null);
        mCursor.moveToFirst();
        if (mCursor.getCount() > 0){
            if (!mCursor.isAfterLast()) {
                do {
                    MakananBelanja entry = new MakananBelanja();
    ...
    public ArrayList<DestinasiMbox> getAllDestinasiMbox() {
        ArrayList<DestinasiMbox> list = new ArrayList<>();
        Cursor mCursor = readableDatabase.rawQuery("SELECT * FROM " + DBHandler.TABLE_DESTINASI_MBOX, null);
        mCursor.moveToFirst();
        if (mCursor.getCount() > 0){
            if (!mCursor.isAfterLast()) {
                do {
                    DestinasiMbox entry = new DestinasiMbox();
